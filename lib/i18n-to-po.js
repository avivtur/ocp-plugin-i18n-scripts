...
function resolveLanguageDir(language) {
  const config = loadConfig();
  return config.languageAliases[language] || language;
}
...
function consolidateWithExistingTranslations(filePath, fileName, language) {
  ...
  const dirLang = resolveLanguageDir(language);
  const existingTranslationsPath = path.join(localesDir, dirLang, `${fileName}.json`);
